...
> **Read this honestly.** "AFTER" means *Claude Code + an AI-for-science model wrote, ran, and benchmarked the workflow*. The headline is the collapse in **human effort** (days of bespoke coding → one command in minutes) and the **breadth of rigor** added — *not* a claim that a neural net always beats classical statistics. Across the **11 experiments** below the verdicts are mixed *on purpose*: clean AFTER wins where structure rewards it (hydrology +0.56 NSE, air-quality −5.3 RMSE, conformal calibration ~7× tighter, hard-mode land cover +0.36 acc, spatial interpolation −0.70 RMSE, SDM +0.19 suitability correlation, biodiversity recall, amortized inference **~1451× faster** per dataset at scoring), a **rigor/uncertainty** win (extremes), and honest **ties / classical-wins** where they belong (1-step forecasting: **SARIMA beats the LSTM**; easy land cover: RF = CNN; kriging keeps an uncertainty surface RF lacks; amortized intervals are wider than MCMC). That mix *is* the point — the agent lets you find out fast, fairly, and reproducibly.
...
| 06 | Spatial interpolation (geostatistics) | IDW **4.94** → ordinary kriging/GP **2.91** RMSE | RF + elevation covariate: **2.20** RMSE | uses a covariate the coords-only smoother can't follow | **AFTER win with a caveat** — **−0.70 RMSE (~24%)** over a real kriging baseline, *but* kriging keeps a calibrated variance surface → regression-kriging is the true hero |
| 07 | Air-quality PM2.5 nowcast | persistence / ARIMA RMSE **17.4** | gradient boosting + weather: **12.1** | meteorology (wind, inversion) the history-only models lack | **Clean AFTER win** — **−5.3 RMSE, +0.30 skill**, spike-F1 0.74→0.85; ablation shows covariates drive it |
...
| 09 | Bayesian hierarchical (partial pooling) | from-scratch MCMC (Metropolis-Hastings): **11.6 s/dataset**, coverage 1.00 | small MLP **amortized over simulations**: train-once 24.5 s, **score 0.008 s** | one-time training cost paid back from the 2nd dataset onward | **~1451× faster at scoring**, coverage parity (≥nominal 0.95); amortized intervals wider (RMSE 0.27 vs 0.15) — honest speed/sharpness tradeoff |
| 10 | Species distribution modeling | logistic GLM + sq terms: AUC **0.705**, Brier **0.124**, suit-corr **0.696** | calibrated GBM with same covariates: AUC **0.728**, Brier **0.122**, suit-corr **0.884** | recovers the elevation × climate niche interaction the GLM can't encode | **AFTER win on all 3** — modest AUC (+0.023), small Brier (−0.002), big suitability-corr (+0.188); GLM still safer for extrapolation |
