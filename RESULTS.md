...
> **Read this honestly.** "AFTER" means *Claude Code + an AI-for-science model wrote, ran, and benchmarked the workflow*. The headline is the collapse in **human effort** (days of bespoke coding → one command in minutes) and the **breadth of rigor** added — *not* a claim that a neural net always beats classical statistics. Across the **9 experiments** below the verdicts are mixed *on purpose*: clean AFTER wins where structure rewards it (hydrology +0.56 NSE, air-quality −5.3 RMSE, conformal calibration ~7× tighter, hard-mode land cover +0.36 acc, spatial interpolation −0.70 RMSE, biodiversity recall), a **rigor/uncertainty** win (extremes), and honest **ties / classical-wins** where they belong (1-step forecasting: **SARIMA beats the LSTM**; easy land cover: RF = CNN; and kriging keeps an uncertainty surface RF lacks). That mix *is* the point — the agent lets you find out fast, fairly, and reproducibly.
